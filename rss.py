...
for i in range(0, len(sites)):
    ...
    for j in range(0, len(raw_links)):
        ...
        if site_id not in downloaded:
            ...
            if site not in ['ttg','openCD','beAst']:
                subprocess.Popen("/usr/bin/deluge-console \"connect 127.0.0.1:58846 ; add -p /home/tobox/down/%s/%s %s/%s/%s.torrent\"" % (now_date, file_name, base_path, now_date, file_name), shell=True)
            else:
